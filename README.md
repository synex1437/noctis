...
| Files changed while the session waited | The checkpoint fingerprints the working tree: `git status`, the commit `HEAD` points at, and the size and modification time of up to 2000 paths: the files `git status` lists first, then the files inside a folder it lists as untracked. If any of it differs at resume — a file edited again after the session had already changed it, a commit, a pull — Claude is told that files it read or edited before the pause may have changed (`wait.workspaceGuard`). `checkpoint.gitSnapshot` can pin uncommitted changes to tracked files as a hidden git ref. |
