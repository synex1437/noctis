...
Then ask one more question and wait for the answer (skip it if `$ARGUMENTS` already contains `--router`): should research and writing prompts — web searches, comparisons, summaries, texts that need no code — go to the `noctis:lite` subagent, so the main session keeps its context for the code? The router stays off unless the user says yes: `--router on` for yes, `--router off` for no.
