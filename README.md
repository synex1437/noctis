...
- **Needs Claude Code's early-access function hooks:** `"env": {"CLAUDE_CODE_ENABLE_FUNCTION_HOOKS": "1"}` in `~/.claude/settings.json` (tested with Claude Code 2.1.281). That switch loads the hooks module of every installed plugin, not only noctis's. Without it noctis works as before, and compaction is Claude Code's alone.

Each compaction noctis trimmed adds a line to `~/.claude/noctis/compact.log`. To turn it off: `"compaction": {"lean": false}` in `~/.claude/noctis/config.json`; the other keys are in [docs/REFERENCE.md](docs/REFERENCE.md).
