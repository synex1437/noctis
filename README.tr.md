...
- **Claude Code'un erken erişimdeki fonksiyon hook'larını ister:** `~/.claude/settings.json` içinde `"env": {"CLAUDE_CODE_ENABLE_FUNCTION_HOOKS": "1"}` (Claude Code 2.1.281 ile denendi). Bu anahtar yalnızca noctis'in değil, kurulu her eklentinin hook modülünü yükler. Anahtar olmadan noctis eskisi gibi çalışır; sıkıştırma yalnızca Claude Code'undur.

noctis'in budadığı her sıkıştırma `~/.claude/noctis/compact.log` dosyasına bir satır ekler. Kapatmak için: `~/.claude/noctis/config.json` içinde `"compaction": {"lean": false}`; diğer anahtarlar [docs/REFERENCE.md](docs/REFERENCE.md) içinde.
