...
After uninstalling, check `permissions.defaultMode` and `env.CLAUDE_CODE_EFFORT_LEVEL` in `settings.json`: uninstall puts back the value setup replaced — the one you had before the first setup, however often setup ran since (a profile switch that changes the effort, say), or one you set yourself between two setup runs. A value you changed after the last setup run stays. Uninstall still removes a permission mode you had already set to exactly what setup writes.
