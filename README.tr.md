...
- **Claude Code'un erken erişimdeki fonksiyon hook'larını ister:** `~/.claude/settings.json` içinde `"env": {"CLAUDE_CODE_ENABLE_FUNCTION_HOOKS": "1"}` (Claude Code 2.1.281 ile denendi); eksikse setup yazar, kaldırma geri alır. Bu anahtar yalnızca noctis'in değil, kurulu her eklentinin hook modülünü yükler. `/noctis:setup --no-lean` anahtara dokunmaz ve yalın sıkıştırmayı kapatır (`compaction.lean: false`; sonraki setup çalışmaları bunu korur). Anahtar olmadan noctis eskisi gibi çalışır; sıkıştırma yalnızca Claude Code'undur.

noctis'in budadığı her sıkıştırma `~/.claude/noctis/compact.log` dosyasına bir satır ekler. Kapatmak için: `~/.claude/noctis/config.json` içinde `"compaction": {"lean": false}`; diğer anahtarlar [docs/REFERENCE.md](docs/REFERENCE.md) içinde. Türü ya da aralığı yanlış bir anahtar eklentiyle gelen değerine döner; `noctis status` ve `noctis doctor` onu adıyla söyler.
