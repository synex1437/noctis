...
- **Works through a list of jobs** without stopping to ask: `/noctis:start jobs.md` takes them from a file you name and says how many it found, a long prompt with several tasks becomes such a list by itself, and a `TASKS.md` you trust drives every session — priorities, dependencies, GitHub issues.
...
- **Compacts lean**: noctis asks Claude Code for the compaction between turns at 70 % context instead of at its later threshold, and while Claude Code's prompt caching is off it trims old tool output from what the summary reads ([Lean compaction](#lean-compaction); it runs on Claude Code's early-access function hooks, which setup turns on).
...
**Requirements:** Claude Code 2.1.251 or newer (lean compaction needs Claude Code's function hooks and was tested with 2.1.281); Windows, macOS or Linux; a Pro or Max subscription for the limit guard. On an API key there are no usage windows, so the guard has nothing to act on: the status line shows *waiting for limit data*, and a note at session start, at most once a day, says there is no OAuth token. Queue mode, the router and the 529/5xx retry still work. No Node, no Git Bash, no compiler. Every profile runs on models every paid plan includes — Opus 5.5 and Haiku 4.5 — so none of them needs Max; none uses Fable, which bills usage credits on Pro.
...
| A scheduled task, only while a resume is pending | Task Scheduler `Noctis-…` (can wake the PC), launchd `com.synex.noctis.…`, systemd `noctis-…` (each gets the PATH, display, certificate and proxy variables of the session that paused, so the relaunch finds `claude` as that session did; the proxy variables, which can hold a password, reach the runner through a file only you can read in `~/.claude/noctis/proxies/`, never through a command line or the plist, and a runner Task Scheduler starts gets all of these variables through that file) — or, where none of these exists, a background `noctis sleeper` process (it cannot wake the machine and does not survive a reboot). Next to a scheduled task, a background `noctis sleeper --watch` process checks for an early reset every 5 minutes until the resume time (`wait.earlyResetPollMinutes: 0` turns it off). | `noctis cancel`; `noctis install --uninstall` cancels every pending one; `"alarm": {"wakePc": false}` stops the wake |
...
<details>
<summary>The format of a job list for `/noctis:start` or a `TASKS.md`, plus priorities, tags, dependencies and GitHub issues.</summary>
...
`noctis queue import` appends open GitHub issues as `- [ ] (P1) #123 Title` through the `gh` CLI (`owner/name#123` with `--repo owner/name`) — priority from `P0`–`P9` or `priority: high` labels, idempotent. It takes only the issues you opened (the account `gh` is logged in with on the repository's host), or with `--author login1,login2` only those authors' issues (`@me` names that account), so a stranger's issue on a public repository never becomes a queue item; it says how many it left out and whose. And `queue.github.closeOnDone` closes an issue once the items that start with its reference are all ticked; a `#123` further along an item closes nothing. A close that `gh` refuses (no login, no network) is tried again at the next stops, three times in all, and then left with gh's reason in `errors.log`.

A check between items is opt-in: with `"queue": {"verifyCommand": "go test ./..."}` in `~/.claude/noctis/config.json`, noctis runs that command in the project folder whenever an item was ticked, before Claude gets the next one and before the queue counts as finished; a failure sends Claude back to fix it with the end of the output, and after `queue.verifyAttempts` failures in a row (2) the queue is held until the command passes, and it runs again at the first stop after you type a prompt. With `queue.github.closeOnDone`, an issue closes only once the command has passed with its items ticked. The command comes only from your own config file, never from `TASKS.md` or a prompt, and runs without a permission prompt.
...
| You paste a long prompt with several tasks | noctis writes the checklist itself, in its own folder, and runs it: `☰ 5-step job detected`. When the jobs are written as sentences rather than as a list, Claude first copies them into that checklist in your own words, and noctis refuses any job whose words are not in your prompt, so nothing you did not ask for is queued. Claude Code would ask before Claude ticks a file there, so noctis answers that one request itself. Short prompts, questions and single tasks are left alone; `/noctis:stop` ends the job early, and `queue.auto: false` turns this off. |
