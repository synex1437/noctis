...
| A prompt is research or writing | It goes to the `noctis:lite` subagent — Opus 5.5 · xhigh by default, Sonnet 5 on `economy` — so the main session keeps its context for the code. It may save only text documents, and never `CLAUDE.md`, the queue files, or anything under `.claude/`, the account folder (`CLAUDE_CONFIG_DIR`) or the plugin's own folder. File search (Explore) and the `noctis:digest` subagent for long test and build output run on Haiku 4.5. |
